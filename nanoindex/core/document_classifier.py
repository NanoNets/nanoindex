...
def classify_from_markdown(markdown: str) -> str:
    ...
    kv_count = sum(1 for line in lines if re.match(r"^[A-Za-z][^:]{1,40}:\s", line))
    total = len(lines)
...
async def classify_with_api(file_id: str, client) -> dict:
    ...
    categories = [
        {
            "name": "report",
            "description": "Reports, filings, manuals, papers with sections and headings",
        },
        {
            "name": "table",
            "description": "Tables, ledgers, claims, transaction lists, spreadsheet-like data",
        },
        {
            "name": "form",
            "description": "Invoices, receipts, tax forms, applications, key-value documents",
        },
    ]
    try:
